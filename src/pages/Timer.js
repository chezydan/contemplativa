...
function Timer() {
    ...
    const [timeVector, setTimeVector] =useState([3,5,12,20 ,30,45]);
...
function TimerSet({num}){
  ...
  return (<div >
   ...
   <div></div>
      <div> <label className='lblTimer'
       htmFor='setTimer' > minutos</label>
     <input type='number'
     id='setTimer'
      ...
      <div></div>
        <button className='btn-timer' onClick={()=>{setRunning(!running)}}>
          {!running? 'Activar': 'Detener'}</button>
          <div></div><br></br>
